...
**Fork:** `rc` — 73 commits ahead of `063359e` (force-pushed to `origin/rc`).
...
73 commits on `rc` (batches 1–23). Key milestones:
...
Tests: 549 → 886. LOC: 12,202 → ~13,300. Corpus: 11 → 15 specs. Source files: 28 → 36.
...
Audit of 16 repository files in `app-business` consuming the generated client revealed ~2,400 lines of boilerplate the generator could eliminate. See `~/Dev/app-business/packages/business_api/HANDOFF.md` for full details with file:line references. Summary of generator-actionable items:

| Priority | Fix | Effort | Impact |
|----------|-----|--------|--------|
| P1 | Barrel re-exports `Provider` from runtime, clashing with Riverpod — stop re-exporting clashing symbols | Low | High (19 files need `hide Provider`) |
| P1 | Boolean query params emitted as enum classes — detect `type: boolean` and emit `bool?` | Low | Medium (14 awkward ternaries) |
| P1 | `type: integer` fields emitted as `num` — emit `int` when spec says integer | Low | Medium (10+ `.toInt()` calls) |
| P2 | Error types lack shared interface — emit `ApiTypedError` so consumers avoid `dynamic` cast | Medium | High (every error path) |
| P2 | Discriminator `type:` param redundantly required in variant constructors | Low | Medium (8+ tautological params) |
| P3 | `OneOf7.a`/`.b`/`.f` positional union constructors are opaque — generate named constructors | Medium | Medium (8+ fragile call sites) |
