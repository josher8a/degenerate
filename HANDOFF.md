...
- Batch 26: Fixes from the batch-25 diff review (three reviewers over 9bff3c08..77c3eb3; findings in `outputs/CODEBASE-REVIEW.md`). **Regressions in batch 25**: `_elementFromJson` null-guard now keys off the element's own nullability (resolved-target guard produced non-compiling `List<X?>` mappers vs `List<X>` declarations); default-response error variant suffix runs through `safeTypeName` (collection bodies emitted `Error$List<X?>` class names); path-param `{name}` matching happens on the raw path with literal segments escaped per-segment (pre-escaping broke `$`/`'` param names → silently unsent); import analyzer mirrors the emitter's empty-content→void early return (was importing unused types); duplicate implicit discriminator keys restored to last-wins + warning (putIfAbsent guard had silently flipped openai's `'message'` arm), explicit keys still win, and inline-variant keys derive before lowering (no orphan types); ref_inliner pre-registers root wrapper refKeys before walking (pre-seeding alone created `Owner`/`Owner2` duplicate splits for shared external files). **R7/H9 — sealed untagged unions now dispatch every variant kind**: `fromJson(Object? json)` handles enums (wire-type + `isUnknown` membership rejection so unknown values fall through), primitives (incl. duration-as-millis and string-encoded kinds), lists/maps (decoded off the promoted value), and objects (`is Map` + `canParse`); refs to sealed unions pass raw JSON instead of a Map cast; fromJson list mappers and OneOf `fromX:` parsers emit tearoffs (`X.fromJson`) where possible; roundtrip fixture reclaim analysis accounts for non-object earlier variants via coarse wire-kind disjointness. This fixes the recursive-oneOf shape (speakeasy `RecursiveOneOfValue`: string/array/map payloads all parse and round-trip) and cloudflare's mixed enum+object unions (`zones_setting_value`). New e2e property test: recursive mixed oneOf compiles + round-trips all three wire shapes.
- Batch 27: Merged upstream/main v0.4.2 (Bo merged our PR #10 as upstream #12; he fixed issues #7/#8/#9 himself). Fork implementations kept everywhere (they already covered his fixes); adopted his better #8 layering — apiKey schemes missing `name`/`in` normalize at lowering with warnings (`name` defaults to the scheme key; a nameless scheme previously emitted a null map key) and the emitter's apiKey arms all default the parameter name. New: `Generator.logs` exposes collected diagnostics for tests/programmatic callers. Upstream's tests kept, adapted to fork emission forms. Note: `make update_examples` output no longer analyzes against the flat `example/` layout (package-name imports + mini-barrels vs workspace-owned package name) — examples left at committed state, regen needs a rethink.
- Batch 28: Backlog burn-down (param wire formats + runtime). **H20** — `RetryInterceptor` rethrows `CancelledException` immediately (an executor timeout or user cancel was retried through the whole backoff schedule, silently extending every timeout). **H14** — required-but-nullable query params get a null guard on the list/object/map/allowReserved serialization paths (previously non-compiling output). **H15** — scalar `DateTime` params serialize `toIso8601String()` (was debug `toString()`, rejected by servers), bytes params `base64Encode` (was a list dump), durations as milliseconds; import analyzer adds `dart:convert` for bytes params (also fixes M3).
- Batch 29: Naming/dedup invariants (H17/H18/H19 + low #7). Structural fingerprints now include `IrField.defaultValue` (drives ctor defaults/required-ness/nullability/toJson guards), the Dart field name (JSON-key↔field mapping is declaration-order dependent for same-camelization keys), and `IrEnum.defaultValue`. Pass-2 union dedup keys discriminated unions by `(discriminatorProperty, sorted key=variant pairs)` — same variants under a different discriminator no longer merge. `collectTypeRefs` walks `IrObject.additionalProperties`, so AP value types survive `--tags` tree-shaking and appear in mini-barrels. Corpus impact: ~50 wrongly-merged types un-merged (e.g. cloudflare `Prompt` variants that shared one class across genuinely different field sets), AP-reachable exports added to mini-barrels.
- Batch 30: H16 — object/array styled-parameter serialization. Path params with array/object/map types serialize per the `simple` style (comma-joined items; `k,v` pairs explode=false, `k=v` pairs explode=true) instead of the Dart debug `toString()`; `$ref`'d object/array query params resolve through the registry before the style switch (previously fell to the scalar default and stringified the whole object); explode=true emits one query entry per item for every style (pipeDelimited/spaceDelimited exploded arrays were joined); explode=false object/map pair joins honor the style delimiter (`|`, space). speakeasy's parameters_api debug-string URLs are gone.
- Batch 31: H10 + H11. **AnyOf wire symmetry** — `toJson` is now `dynamic` and mirrors `fromJson`: scalar variants (primitives, enums, extension types) early-return their raw wire value instead of being wrapped under invented field-name keys, the `dynamic` catch-all returns after typed scalars, union variants return their own `toJson`, collection variants return raw (consumer-constructed), and only object-like variants merge into the map spread. `fromJson(toJson(x))` holds and clients no longer send objects where the schema says scalar (cloudflare `workers_kv_any`). Int/double-valued enum variants in anyOf parse from numeric wire values (was a non-compiling `json is String` check). **Synthetic disc enum collisions** — the synthetic name avoids registry-owned names: when the spec defines the shared discriminator enum under the natural `${Union}${Prop}` name the getter reuses it instead of re-declaring the class, and genuinely colliding synthetic enums get a `Disc` suffix.
- Batch 32: H21 — greedy OneOf variant dispatch. `OneOf.parse` takes a `greedy` index set; greedy variants (free-form maps, `dynamic`, AP-capturing objects — detected statically by the generator) only compete when no typed variant parses. Previously a map-like variant always won on key coverage the moment the server added a field, flipping the parsed variant on a backward-compatible change. 243 parse sites in the corpus carry hints.
- Batch 33: Port of upstream review follow-ups (Bo's `e50f4975` on his wire-formats branch — fixes to bugs in our batch-30 styled-param code). Per RFC 6570 only VALUES are percent-encoded in simple-style path params — the comma/equals delimiters stay literal (previously the whole joined value went through `Uri.encodeComponent`, sending `3%2C4`); exploded map keys are encoded too. Nullable element types compile: `List<T?>` path/query params use `.nonNulls`, nullable map values skip null entries, required-but-nullable object fields get presence guards (`fieldNullable = !isRequired || type.isNullable`) and joined objects if-spread their pairs. Import analysis adds `dart:convert` for bytes FIELDS inside styled object params (signature references the model, not Uint8List). Duration query scalars are String-typed (`inMilliseconds.toString()`).

Tests: 549 → 886 → 924 → 937 (main suite). LOC: 12,202 → ~13,500. Corpus: 11 → 15 specs. Source files: 28 → 36.
