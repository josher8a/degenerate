...
**Fork:** `rc` — 34 commits ahead of `origin/rc`, 55 commits ahead of `063359e`.
...
55 commits on `rc` (batches 1–20). Key milestones:
...
Pipeline in `generator.dart` (10 stages): parse → inline → normalize+lower (bundled in `_lowerToIr`) → name resolution → filter → typed-param rewrite (gated) → validate → union analysis → emit → write. The `generator.dart:1` doc comment is stale — omits Name Resolution, Filter, Validate, and union analysis.

### Test coverage — new findings

| Module | LOC | Direct Tests | Gap |
|--------|----:|:---:|-----|
| `emitter/anyof_emitter.dart` | 262 | 0 | Only snapshot comparison; no dedicated unit tests |
| `emitter/roundtrip_emitter.dart` | 462 | 0 | Only end-to-end via wire gate |
| `emitter/negative_fixture_emitter.dart` | 334 | 0 | Only end-to-end via wire gate |
| `emitter/untagged_union_emitter.dart` | 286 | 2 | Thin for its size; relies on indirect coverage |
| `emitter/emit_utils.dart` | 903 | ~15 | Disproportionate for largest utility file |

Property test gaps confirmed: no randomized coverage for extension types, error unions, negative fixture synthesis, discriminated unions with mapping, or `--emit-typed-params` with diverse param types.
...
| Finding | Severity | Location |
|---------|----------|----------|
| `RegExp` in `operation_lowerer.dart:255` constructed per parameter | Low | Trivial hoist to file scope |
...
New minor finding: `_formatSamples` keys in `roundtrip_emitter.dart` must stay in sync with `_formatTypeNames` in `ir_mapper.dart` — no test validates this correspondence.

### Updated prioritized action list

| Priority | Dimension | Fix |
|----------|-----------|-----|
| P1 | DX | Log warning on silent type/param drops from name collision (`generator.dart:315,325,688`) |
| P1 | DX | Show IR validation warnings by default, not gated behind `--verbose` (`generator.dart:169`) |
| P1 | Tests | Unit tests for `anyof_emitter.dart` (262 LOC, zero dedicated tests) |
| P2 | DX | Explicit `on UnsupportedError` catch + stack trace dump in `--verbose` (`bin/degenerate.dart:240`) |
| P2 | Tests | Direct tests for `roundtrip_emitter.dart` (462 LOC) + `negative_fixture_emitter.dart` (334 LOC) |
| P2 | Tests | More tests for `untagged_union_emitter.dart` (286 LOC, only 2 dedicated) |
| P2 | Tests | Property test gaps: extension types, error unions, discriminated unions with mapping |
| P2 | Generated | Propagate field-level doc comments from spec `description`/`example` |
| P2 | Complexity | Test asserting `_formatSamples.keys == _formatTypeNames.keys` |
| P3 | Performance | Hoist `RegExp` in `operation_lowerer.dart:255` to file scope |
| P3 | Generated | Add `// Source:` path comments to generated API files |
| P3 | DX | Update stale doc comment at `generator.dart:1` |
| P3 | Generated | `$Unknown` defensive map copy (changes output across 15 specs) |
