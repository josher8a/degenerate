...
sealed class AddParticipantError {
  const AddParticipantError();

  int get statusCode;

  factory AddParticipantError.fromResponse(ApiResponse response) {
    try {
...
final class AddParticipantError$500 extends AddParticipantError {
  const AddParticipantError$500(this.error);
  final RealtimekitGenericErrorResponse error;
  @override
  int get statusCode => 500;
}

final class AddParticipantError$Unknown extends AddParticipantError {
  const AddParticipantError$Unknown(this.statusCode, this.rawBody);
  @override
