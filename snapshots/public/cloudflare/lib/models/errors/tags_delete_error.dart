...
import 'package:pub_cloudflare/models/response_common_failure59.dart';

sealed class TagsDeleteError {
  const TagsDeleteError();

  int get statusCode;

  factory TagsDeleteError.fromResponse(ApiResponse response) {
    try {
...
final class TagsDeleteError$412 extends TagsDeleteError {
  const TagsDeleteError$412(this.error);
  final ResponseCommonFailure59 error;
  @override
  int get statusCode => 412;
}

final class TagsDeleteError$Unknown extends TagsDeleteError {
  const TagsDeleteError$Unknown(this.statusCode, this.rawBody);
  @override
