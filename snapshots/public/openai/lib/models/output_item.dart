...
import 'package:degenerate_runtime/degenerate_runtime.dart';import 'apply_patch_call_output_status.dart';import 'apply_patch_call_status.dart';import 'apply_patch_tool_call.dart';import 'apply_patch_tool_call_operation.dart';import 'apply_patch_tool_call_output.dart';import 'code_interpreter_tool_call.dart';import 'code_interpreter_tool_call_outputs2.dart';import 'compaction_body.dart';import 'computer_action.dart';import 'computer_call_safety_check_param.dart';import 'computer_tool_call.dart';import 'custom_tool_call.dart';import 'file_search_tool_call.dart';import 'file_search_tool_call_results2.dart';import 'function_shell_action.dart';import 'function_shell_call.dart';import 'function_shell_call_environment.dart';import 'function_shell_call_output.dart';import 'function_shell_call_output_content.dart';import 'function_tool_call.dart';import 'image_gen_tool_call.dart';import 'local_shell_exec_action.dart';import 'local_shell_tool_call.dart';import 'mcp_approval_request.dart';import 'mcp_list_tools.dart';import 'mcp_list_tools_tool.dart';import 'mcp_tool_call.dart';import 'message_phase.dart';import 'output_message.dart';import 'output_message_content.dart';import 'reasoning_item.dart';import 'reasoning_text_content.dart';import 'summary_text_content.dart';import 'tool.dart';import 'tool_search_call.dart';import 'tool_search_execution_type.dart';import 'tool_search_output.dart';import 'web_search_tool_call.dart';import 'web_search_tool_call_action.dart';sealed class OutputItem {const OutputItem();
...
factory OutputItem.message({required String id, required OutputMessageRole role, required List<OutputMessageContent> content, MessagePhase? phase, required OutputMessageStatus status, }) { return OutputItemMessage(OutputMessage(type: 'message', id: id, role: role, content: content, phase: phase, status: status)); }

/// Build the `file_search_call` variant.
factory OutputItem.fileSearchCall({required String id, required FileSearchToolCallStatus status, required List<String> queries, List<FileSearchToolCallResults2>? results, }) { return OutputItemFileSearchCall(FileSearchToolCall(type: 'file_search_call', id: id, status: status, queries: queries, results: results)); }

/// Build the `function_call` variant.
factory OutputItem.functionCall({String? id, required String callId, String? namespace, required String name, required String arguments, FunctionToolCallStatus? status, }) { return OutputItemFunctionCall(FunctionToolCall(type: 'function_call', id: id, callId: callId, namespace: namespace, name: name, arguments: arguments, status: status)); }
...
factory OutputItem.reasoning({required String id, String? encryptedContent, required List<SummaryTextContent> summary, List<ReasoningTextContent>? content, ReasoningItemStatus? status, }) { return OutputItemReasoning(ReasoningItem(type: 'reasoning', id: id, encryptedContent: encryptedContent, summary: summary, content: content, status: status)); }
...
factory OutputItem.codeInterpreterCall({required String id, required CodeInterpreterToolCallStatus status, required String containerId, required String? code, required List<CodeInterpreterToolCallOutputs2>? outputs, }) { return OutputItemCodeInterpreterCall(CodeInterpreterToolCall(type: 'code_interpreter_call', id: id, status: status, containerId: containerId, code: code, outputs: outputs)); }

/// Build the `local_shell_call` variant.
factory OutputItem.localShellCall({required String id, required String callId, required LocalShellExecAction action, required LocalShellToolCallStatus status, }) { return OutputItemLocalShellCall(LocalShellToolCall(type: 'local_shell_call', id: id, callId: callId, action: action, status: status)); }
