...
W when<W>({required W Function() enabled, required W Function() disabled, required W Function() notSet, required W Function(String value) $unknown, }) { return switch (this) {
      SecretScanningGenericSecrets$enabled() => enabled(),
      SecretScanningGenericSecrets$disabled() => disabled(),
      SecretScanningGenericSecrets$notSet() => notSet(),
      SecretScanningGenericSecrets$Unknown(:final value) => $unknown(value),
    }; } 
...
W maybeWhen<W>({required W Function(String value) orElse, W Function()? enabled, W Function()? disabled, W Function()? notSet, W Function(String value)? $unknown, }) { return switch (this) {
      SecretScanningGenericSecrets$enabled() => enabled != null ? enabled() : orElse(value),
      SecretScanningGenericSecrets$disabled() => disabled != null ? disabled() : orElse(value),
      SecretScanningGenericSecrets$notSet() => notSet != null ? notSet() : orElse(value),
      SecretScanningGenericSecrets$Unknown(:final value) => $unknown != null ? $unknown(value) : orElse(value),
    }; } 
