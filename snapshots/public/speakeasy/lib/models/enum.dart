// GENERATED CODE - DO NOT MODIFY BY HAND
// Source: #/components/schemas/Enum

import 'package:degenerate_runtime/degenerate_runtime.dart';/// A string based enum
@immutable final class Enum {const Enum._(this.value);

factory Enum.fromJson(String json) { return switch (json) {
  'one' => one,
  'two' => two,
  'three' => three,
  'four_and_more' => fourAndMore,
  _ => Enum._(json),
}; }

static const Enum one = Enum._('one');

static const Enum two = Enum._('two');

static const Enum three = Enum._('three');

static const Enum fourAndMore = Enum._('four_and_more');

static const List<Enum> values = [one, two, three, fourAndMore];

final String value;

String toJson() { return value; } 
/// The Dart identifier name for this value, or the raw value if unknown.
String get name { return switch (value) {
  'one' => 'one',
  'two' => 'two',
  'three' => 'three',
  'four_and_more' => 'fourAndMore',
  _ => value,
}; } 
/// Whether this value is unknown (not defined in the OpenAPI spec).
bool get isUnknown { return !values.contains(this); } 
@override bool operator ==(Object other) => identical(this, other) ||
    other is Enum && other.value == value;

@override int get hashCode => value.hashCode;

@override String toString() => 'Enum($value)';

 }
