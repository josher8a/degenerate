...
// Source: #/components/schemas/AmountDetailsParam (inline: LineItems > Variant1 > PaymentMethodOptions > Klarna)
...
if (imageUrl$ != null) {
  if (imageUrl$.runes.length > 4096) { errors.add('imageUrl: length must be <= 4096'); }
}
final productUrl$ = productUrl;
if (productUrl$ != null) {
  if (productUrl$.runes.length > 4096) { errors.add('productUrl: length must be <= 4096'); }
}
final reference$ = reference;
if (reference$ != null) {
  if (reference$.runes.length > 255) { errors.add('reference: length must be <= 255'); }
}
final subscriptionReference$ = subscriptionReference;
if (subscriptionReference$ != null) {
  if (subscriptionReference$.runes.length > 255) { errors.add('subscriptionReference: length must be <= 255'); }
}
return errors; } 
