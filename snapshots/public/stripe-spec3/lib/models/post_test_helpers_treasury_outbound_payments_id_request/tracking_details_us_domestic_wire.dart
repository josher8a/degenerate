...
// Source: #/components/schemas/PostTestHelpersTreasuryOutboundPaymentsIdRequest (inline: TrackingDetails > UsDomesticWire)
...
final chips$ = chips;
if (chips$ != null) {
  if (chips$.runes.length > 5000) { errors.add('chips: length must be <= 5000'); }
}
final imad$ = imad;
if (imad$ != null) {
  if (imad$.runes.length > 5000) { errors.add('imad: length must be <= 5000'); }
}
final omad$ = omad;
if (omad$ != null) {
  if (omad$.runes.length > 5000) { errors.add('omad: length must be <= 5000'); }
}
return errors; } 
